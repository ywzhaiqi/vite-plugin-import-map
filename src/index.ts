...
function PluginImportToMap(options: Options): Plugin[] {
    ...
    const plugins: Plugin[] = [
        {
            ...
            transformIndexHtml(html) {
                ...
                data.forEach(p => p.pathList.map(url => {
                    imports[p.name] = url
                }))
                const jsMap =  !isBuild
                        ? ''
                ...
                const jsCode = !isBuild
                    ? ''
                    : data
                        .map(p => p.pathList.map(url => `<link rel="modulepreload" crossorigin href="${url}">`).join('\n'))
                        .join('\n')
...
export default PluginImportToMap
